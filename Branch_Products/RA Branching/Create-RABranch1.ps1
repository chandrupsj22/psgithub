...
Param(
		...
		foreach ( $project in $settings.project.components.component ) {
			...
			if ($project.sourcetype -eq 'github')    
			{
				...
				elseif($project.ra2only){  
					$project_vars = @{'repo'=$project.name.ToLower();'project'=$project.project;'trunkBranch'=$trunkBranch;'ra2only'='true';}
					$github_projects.add($project_vars)
				}
				...
				elseif($project.ra3only){    
					$project_vars = @{'repo'=$project.name.ToLower();'project'=$project.project;'trunkBranch'=$trunkBranch;'ra3only'='true';}
					$github_projects.add($project_vars)
	...
	foreach ($object in $GitHub_projects){
        ...
        elseif($object.ra2only){
        	$fullbranch="release/2only.$release"
        }
        elseif($object.ra3only){
        	$fullbranch="release/3only.$release"
        }
        else{
            ...
            try {     
				...
				Write-Host -NoNewline "Branching $($object.project)/$($object.repo) from $($object.trunkBranch) to " $fullbranch `n 
			       ...
			       New-Branch -GithubProject $object.project -GithubRepo $object.repo -NewBranchName $fullbranch -BranchFrom $trunkBranch
      Write-Host -NoNewline "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------<<< $($object.repo) >>>" `n
	...
	Write-Host "GITHUB Code branch complete for $release"
